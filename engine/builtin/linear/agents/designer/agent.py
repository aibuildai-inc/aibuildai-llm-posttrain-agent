"""DesignerAgent -- the role that proposes the linear chain's one starting design.

The Designer produces ``DesignerOutput`` (fresh DesignPlans). Every later link
of the chain is proposed by the Worker that just ran, not by a separate role.
"""
...
from engine.builtin.linear.agents.designer.io import DesignerInput, DesignerOutput
...
class DesignerAgent(Agent[DesignerInput, DesignerOutput]):
    ...
    async def verifier_invocations(
        ...
        """The reviewer that reads the plan, when the operator asked for one.
        ...
        schema can make: whether the plan is worth building."""
...
# so it carries them. Its verdict reason is shown verbatim to the Designer, a
...
#. Write/Edit reach only SCRATCH, so no plan file can be edited into
# agreement with its verdict.
