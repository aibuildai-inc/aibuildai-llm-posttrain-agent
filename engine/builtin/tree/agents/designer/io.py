...
class DesignPlan(WorkflowBaseModel):
    model_config = ConfigDict(
        ...
        design; the Crossover emits one per offspring.

        Revisions of an existing plan are NOT DesignPlans — they are
        ``RevisionProposal`` instances, which describe a *diff* against the
        revised design's effective design rather than a fresh full plan. See
        ``RevisionProposal`` in ``engine/work_unit/agent/reviser/io.py``."""
            )
        },
    )
    ...
    validation_strategy: str = Field(
        ...
        "the next Reviser. Read it as guidance for the next Reviser, not as a "
...
class DesignerOutput(SuccessfulOutput):
    ...
    feedback: str = Field(
        ...
        description=(
            ...
            "did NOT explore (so the next Reviser knows the unexplored "
            "search space)."
        ),
    )
