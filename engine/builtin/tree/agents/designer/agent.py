"""DesignerAgent — the role that proposes designs from scratch.

The Designer produces ``DesignerOutput`` (a list of fresh DesignPlans). The Reviser (``ReviserAgent`` in engine.builtin.tree.agents.reviser.agent) produces ``ReviserOutput`` (diffs against the completed design it revises). Their output schemas and prompts are owned by their concrete classes.
"""
...
from engine.builtin.tree.agents.designer.io import (
    DesignerInput,
    DesignerOutput,
    WorkerDesignerOutput,
)

if TYPE_CHECKING:
    pass

DesignerT = TypeVar("DesignerT", bound=Agent)
