"""The nb package's own candidate Composite.
...
Score Program measures the directory it filled. There is no Coder, no Training
Program, no Reviser, no Judge, no Selector, and no Aggregator. A fresh design
and a revision of one are the SAME operation here, told apart by the business
facts on this Input rather than by two Composite classes that forward to one
body. Everything a candidate needs arrives on its own frozen Input: it reads
no configuration, resolves no capability by role name, and asks the run
...
from engine.builtin.nb_tree.agents.worker.io import WorkerInput
from engine.builtin.nb_tree.io import WorkerCandidateInput, WorkerCandidateOutput
...
class CompleteWorker(Composite[WorkerCandidateInput]):
    """One complete Worker attempt in this tree, scored after it finishes."""
