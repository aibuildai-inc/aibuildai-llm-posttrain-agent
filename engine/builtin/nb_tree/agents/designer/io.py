"""Input and Output records for the nb_tree package's own designer role.

Copied from ``engine.builtin.tree.agents.designer.io`` and trimmed to the
Worker variant this package actually uses: ``WorkerDesignPlan`` and
``WorkerDesignerOutput``, plus the plain ``DesignPlan`` / ``DesignerOutput``
bases they specialize. Local to this package -- another package may spell a
field the same way with its own meaning."""
...
class DesignPlan(WorkflowBaseModel):
    model_config = ConfigDict(
        ...
        A revision of an existing plan is NOT a DesignPlan -- it is a
        ``WorkerRevision``, which describes a diff against the revised
        Worker's own plan rather than a fresh full plan. See
        ``WorkerRevision`` in ``engine/builtin/nb_tree/agents/worker/io.py``."""
            )
        },
    )
    ...
    validation_strategy: str = Field(
        ...
        "the next revision. Read it as guidance for the next revision, not as a "
        "prescription for an in-training validation loop: the design writes its "
        "output once and the task's own score program turns that output into the "
        "design's score afterwards."
    )
    ...
    requested_gpus: int = Field(
        ...
        "model is starved at too low a count. Zero uses no GPU.",
    )
...
class WorkerDesignPlan(DesignPlan):
    """One complete starting plan for a Worker candidate."""

    model_config = ConfigDict(
        json_schema_extra={
            "description": "One complete starting plan for a Worker candidate."
        }
    )
    name: str
    architecture: str
    ...
    validation_strategy: str = Field(
        description=(
            "How the Worker should check the design with visible data and "
            "what evidence should guide a later revision. Do not assume access "
            "to the framework score during the Worker session."
        )
    )
    method_steps: tuple[str, ...]
    requested_gpus: int = cast(int, Field(..., ge=0))
    design_notes: str = cast(str, Field(...))
    references: tuple[Citation, ...] = cast(tuple[Citation, ...], Field(...))
...
class DesignerOutput(SuccessfulOutput):
    ...
    feedback: str = Field(
        ...
        description=(
            ...
            "did NOT explore (so the next revision knows the unexplored "
            "search space)."
        ),
    )
...
class WorkerDesignerOutput(DesignerOutput):
    model_config = ConfigDict(json_schema_extra=None)
    plans: tuple[WorkerDesignPlan, ...] = Field(min_length=1)
    feedback: str = cast(str, Field(...))
