"""Input and Output records for the nb package's own designer Agent family.

Copied from the tree package's own designer records and trimmed to the
Worker variant this package actually composes: only ``WorkerDesignPlan`` and
``WorkerDesignerOutput`` exist here. The plain ``DesignPlan`` / ``DesignerOutput``
forms belong to that other package and do not survive in this copy."""
...
class WorkerDesignPlan(WorkflowBaseModel):
    ...
    validation_strategy: str = Field(
        description="How the Worker should check the design with visible data and "
        "what evidence should guide a later revision. Do not assume access to the "
        "framework score during the Worker session."
    )
    ...
    requested_gpus: int = Field(
        ...
        "model is starved at too low a count. Zero uses no GPU. Each WorkUnit owns "
        "its request: Coder and Training may each request this count.",
    )
...
class DesignerInput(AgentInput):
    """What one Worker Designer call needs about the run it designs for.

    Every field is a fact the owning Search already had; nothing here is
    rediscovered by the Designer or by a helper that reads the run state."""
...
class WorkerDesignerOutput(SuccessfulOutput):
    model_config = ConfigDict(
        json_schema_extra={
            "description": "Output from the Designer — a flat list of fresh design plans."
        }
    )
    plans: tuple[WorkerDesignPlan, ...] = Field(
        min_length=1,
        description="List of design plans. Each plan is a complete, "
        "self-contained WorkerDesignPlan with all fields filled in. Propose diverse "
    ...
    def _plans_fit_the_request(self, info: ValidationInfo) -> "WorkerDesignerOutput":
