...
Program, no Reviser, no Judge, no Selector, and no Aggregator. Everything a
candidate needs arrives on its own frozen Input: it reads no configuration,
resolves no capability by role name, and asks the run nothing about work it
...
from engine.builtin.nb.agents.worker.io import WorkerInput
from engine.builtin.nb.io import WorkerCandidateInput, WorkerCandidateOutput
...
class CompleteWorker(Composite[WorkerCandidateInput]):
    """One complete Worker attempt of this chain, scored after it finishes."""
