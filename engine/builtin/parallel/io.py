...
from engine.builtin.parallel.agents.designer.io import DesignPlan
from engine.builtin.parallel.agents.worker.agent import WorkerAgent
from engine.builtin.parallel.agents.worker.io import ParentAttempt, WorkerRevision
...
class ParallelSearchParameters(WorkflowBaseModel):
    """ParallelSearch's own business parameters: the half a user writes.

    The linear chain widened to N chains: ``parallel`` and ``num_designs``
    open up, while one revision per completed link stays fixed, exactly as
    ``linear`` fixes it."""
    ...
    parallel: int = Field(
        gt=0,
        description="max concurrent evaluation chains. This is the "
        "concurrency ceiling, NOT the search method. Required; never derived "
        "from host hardware.",
    ...
    num_designs: int | None = Field(
        default=None,
        ge=1,
        description="initial designs the designer proposes (one starting "
        "chain per design). null = the designer decides the count from task "
        "complexity; an int forces exactly that many.",
    )
    num_revisions: Literal[1] = Field(
        default=1,
        description="revision proposals a completed evaluation fans out. "
        "Fixed at 1: each chain continues as one line, exactly like "
        "``linear``, just widened to N chains.",
    )
...
class CandidateGrant(WorkflowBaseModel):
    """What one parallel candidate needs to run and score its own Worker.
    ...
    # A chain ends when its Worker proposes nothing, so the floor is zero and
...
class ParallelSearchInput(ParallelSearchParameters):
    """ParallelSearch's complete immutable Input.
...
class WorkerCandidateInput(WorkflowBaseModel):
    """One complete Worker attempt: a chain's first link, or its next one.
...
class WorkerCandidateOutput(ScoreOutput):
    """The terminal success Output of one scored parallel candidate.
