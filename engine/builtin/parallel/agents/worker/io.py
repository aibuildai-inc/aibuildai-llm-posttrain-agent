"""Input and Output records for the parallel package's Worker."""
...
from engine.builtin.parallel.agents.designer.io import Citation, DesignPlan
...
class WorkerRevision(WorkflowBaseModel):
    ...
    It is guidance, not a script: the next Worker reads its parent's own
    formal score before it starts, and refines this direction from it. There
    is no exact find/replace field, because the Worker that would apply one
    owns the inherited source itself and edits it directly."""
